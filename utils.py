...
class PFNLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.dilated_conv = nn.Conv1d(out_channels * 2, out_channels, kernel_size=3, dilation=2, padding=2)
        self.extra_linear = nn.Linear(out_channels, out_channels)
    ...
    def forward(self,
        ...
        x_avg = x.sum(dim=1) / num_voxels.type_as(inputs).view(-1, 1)
        ...
        x_dilated = self.dilated_conv(x_concat)
        x_dilated = x_dilated.squeeze(2)

        # 线性层恢复维度
        x_restored = self.extra_linear(x_dilated)

        if self.last_vfe:
            return x_restored
        else:
            x_restored = x_restored.unsqueeze(1).repeat(1, inputs.shape[1], 1)
            x_concatenated = torch.cat([x, x_restored], dim=2)
