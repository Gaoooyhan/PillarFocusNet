...
class _Voxelization(Function):
    ...
    def forward(
        ...
        if max_points == -1 or max_voxels == -1:
            ...
            # apply kmeans clustering to each voxel
            kmeans = KMeans(n_clusters=max_points)
            voxel_coords = ((points[:, :3] - np.array(coors_range[:3])) / np.array(voxel_size)).astype(int)
            unique_coords = np.unique(voxel_coords, axis=0)
            num_voxels = len(unique_coords)
            voxels = points.new_zeros(size=(num_voxels, max_points, points.size(1)))
            coors = points.new_zeros(size=(num_voxels, 3), dtype=torch.int)
            num_points_per_voxel = points.new_zeros(size=(num_voxels,), dtype=torch.int)
            for i in range(num_voxels):
                # extract points in the current voxel
                voxel_mask = (voxel_coords == unique_coords[i]).all(axis=1)
                voxel_points = points[voxel_mask, :]

                if len(voxel_points) > max_points:
                    # apply kmeans clustering to downsample points
                    kmeans.fit(voxel_points[:, :3])
                    voxel_points = voxel_points[kmeans.labels_ == kmeans.predict(kmeans.cluster_centers_)]

                # set the representative point as the centroid of the downsampled points
                voxel_center = voxel_points[:, :3].mean(axis=0)
                voxels[i, :len(voxel_points), :] = voxel_points
                coors[i, :] = unique_coords[i]
                num_points_per_voxel[i] = len(voxel_points)

            # select the valid voxels
            voxels_out = voxels[:num_voxels]
            coors_out = coors[:num_voxels]
            num_points_per_voxel_out = num_points_per_voxel[:num_voxels]
