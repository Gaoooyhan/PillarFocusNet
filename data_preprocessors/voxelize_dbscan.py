...
class _Voxelization(Function):
    ...
    def forward(
        ...
        if max_points == -1 or max_voxels == -1:
            ...
            for i in range(voxel_num.item()):
                single_voxel_points = voxels[i, :num_points_per_voxel[i], :]

                if single_voxel_points.shape[0] > max_points:
                    # 使用 DBSCAN 聚类
                    dbscan = DBSCAN(eps=0.5, min_samples=5)  # eps 和 min_samples 可根据需要调整
                    dbscan.fit(single_voxel_points.cpu().numpy())
                    ...
                    for k in unique_labels:
                        if k != -1:  # 忽略噪声点
                            ...
                            xy = single_voxel_points[class_member_mask & core_samples_mask]
                            center = xy.mean(0)
                            centers.append(center)

                    # 将聚类中心作为体素中的点
                    num_clusters = min(len(centers), max_points)
                    centers = np.array(centers)[:num_clusters]
                    voxels[i, :num_clusters, :] = torch.from_numpy(centers).to(points.device)
